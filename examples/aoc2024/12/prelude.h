...
template <typename K, typename V>
i64 hashmap_has_key(universal_hashmap<K, V>* h, K key) {
...
template <typename K, typename V>
void hashmap_remove(universal_hashmap<K, V>* h, K key) {
    if (h->find(key) == h->end()) {
...
template <typename K, typename V>
i64 hashmap_len(universal_hashmap<K, V>* h) {
    return h->size();
}

template <typename K, typename V>
std::vector<K>* hashmap_keys(universal_hashmap<K, V>* h) {
...
template <typename T>
void array_set_len(std::vector<T>* a, i64 new_len) {
    a->resize(new_len);
}
...
template <typename T>
i64 array_contains(std::vector<T>* a, T value) {
...
template <typename T>
void array_remove_at(std::vector<T>* a, i64 index) {
