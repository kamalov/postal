...
use stage02_ast_builder::AstError;
use to_vec::ToVec;
...
fn hadle_ast_builder_error(err: AstError, text: &String, tokenizer: &stage01_tokenizer::Tokenizer) {
    ...
    println!("parse error, line {}, column {}", line_number + 1, column_number + 1);
    println!("");
    print!("{}", left);
    ...
    println!("\x1b[93m{}\x1b[0m, token: \x1b[93m{}\x1b[0m", err.expected, v);
}
...
fn main() {
    ...
    let mut type_checker = stage03_type_checker::TypeChecker::new(&ast_builder);
    type_checker.fill_types_in_ast();

    let mut generator = stage04_code_generator::CodeGenerator::new(&mut ast_builder);
