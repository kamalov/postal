...
struct CodeGenerator<'compiler> {
    ast: &'compiler Ast,
    operators_map: HashMap<String, String>,
    current_function_context: Option<CurrentFunctionContext>,
}

pub fn generate_code(compiler: &Compiler) -> String {
    let mut code_generator = CodeGenerator::new(compiler);
    let text = code_generator.generate_code();
    return text;
}

impl CodeGenerator<'_> {
    fn new(compiler: &Compiler) -> CodeGenerator {
    ...
    fn generate_code(&mut self) -> String {
        ...
        for n in &self.ast.root_nodes.clone() {
            match n {
    ...
    fn generate_function_declaration_params_code(&self, declaration: &FunctionDeclaration) -> String {
        let mut parts = vec![];
