...
void run() {
    ...
    print_int_field(f);
    commands = str_arr_join(arr_slice(lines, i + 1ll, len(lines)), ""s);
    //log(commands)

    auto __expr0 = str_to_chars(commands);
    for (int expr__it0__idx = 0; expr__it0__idx < __expr0->size(); expr__it0__idx++) {
        std::string expr__it0 = (*__expr0)[expr__it0__idx];
        delta = get_delta(expr__it0);
        move_next(f, expr__it0);
    }
    print_int_field(f);
    printf("\n");
